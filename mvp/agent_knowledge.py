...
class AgentKnowledge:
    ...
    def enforce_on_plan(self, plan: List[str], metrics=None) -> Tuple[Dict[str, Any], Tuple[bool, str]]:
        ...
        def _as_text(item: Any) -> str:
            ...
            return str(item)
        plan_str = " ".join(_as_text(p) for p in plan or []).lower()
        ...
        if "terraform" not in plan_str and any("infra" in _as_text(p).lower() for p in plan or []):
